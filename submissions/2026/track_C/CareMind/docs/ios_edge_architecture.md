...
> 状态：设计方案 / 下一阶段实现指南。
> 日期：2026-06-08。
> 当前事实：iPhone 端已经支持完整 App 与云端 Agent 工作流；Android 端侧 Gemma LiteRT 已可演示。iOS Swift Native Bridge 已提交，可做模型下载、校验、删除和 stub XML 输出；真实 iOS 本地大模型 runtime 尚未接入，不能作为当前离线模型能力声明。
...
| iOS native bridge | `frontend/modules/caremind-ios-gemma/ios/CaremindGemmaModule.swift` | 已提交 Swift bridge，先支持模型生命周期和 stub XML 输出 |
| iOS native project | `frontend/ios` | 已提交 iOS 原生工程，供 EAS / Xcode 构建 |
...
当前已按同名接口提交 iOS Expo Swift Module：
...
当前实现使用 Expo Modules API 暴露 `CaremindGemma`，JS facade 通过 `requireOptionalNativeModule("CaremindGemma")` 获取 iOS 模块；Android 继续使用 legacy `NativeModules.CaremindGemma`。
...
- 当前 stub engine 返回 CareMind XML 结构，方便先打通 parser、隐私路由和 UI 状态。
